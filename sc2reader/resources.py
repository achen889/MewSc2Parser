...
class Replay(Resource):
    ...
    def load_details(self):
        ...
        if not details["cache_handles"]:
            ...
            log_info(f"ERROR: details[cache_handles] is {cache_handles}!!")
            #print(f"replay contents: {self.__dict__}")
            return
    ...
    def _format_raw_data_entry(self, data_file):
        data = self.raw_data[data_file]
        size_bytes = len(data)
        size_str = f"{size_bytes} Bytes {size_bytes / 1024:.1f} KB" if size_bytes < 1024 * 1024 else f"{size_bytes / (1024 * 1024):.2f} MB"

        return f" => raw_data[{data_file}] => data_size({size_str})"


    def _read_data(self, data_file, reader):
        data = utils.extract_data_file(data_file, self.archive)
        log_info("=" * 80)
        log_info(f" ~ | {self.__class__.__name__}._read_data: data_file={data_file} reader={reader}")
        if data:
            #print(f" ~ | {self.__class__.__name__}._read_data: data={data}")
            self.raw_data[data_file] = reader(data, self)
            log_info(f" ~ | {self.__class__.__name__}._read_data: Reading data at: {data_file} => \n ~ | {self._format_raw_data_entry(data_file)}")
            log_info("-" * 80)
        elif self.opt["debug"] and data_file not in [
            "replay.message.events",
            "replay.tracker.events",
        ]:
            log_info(f"{0} not found in archive".format(data_file))
            return
